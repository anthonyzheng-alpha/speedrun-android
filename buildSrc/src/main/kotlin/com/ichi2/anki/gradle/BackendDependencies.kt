...
// ../speedrun-android-backend (when `local_backend=true` in local.properties) or as
// the published library from the version catalog.

// Paths within the `speedrun-android-backend` checkout, which sits beside the repo root.
private const val BACKEND_CHECKOUT_DIR = "speedrun-android-backend"
...
private fun Project.addBackendArtifact(
    ...
    if (useLocalBackend) {
        val backendCheckout = File(rootProject.projectDir.parentFile, BACKEND_CHECKOUT_DIR)
